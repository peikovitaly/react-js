...
import { QUANTITY_FIELD } from '@/features/trade/containers/create-order/constants'
...
export const calcTotal: TCalcTotal = ({ fields, pair }) => {
  ...
  try {
    if (fields[QUANTITY_FIELD] && pair.lastPrice) {
      const result = new D(fields[QUANTITY_FIELD]).times(new D(pair.lastPrice)).toFixed()
